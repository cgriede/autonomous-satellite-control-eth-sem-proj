"""Read, validate, and initialize the live project backlog workbook (agent tool).

Canonical location: .cursor/tools/backlog/ — not backend app runtime.
...
DEFAULT_BACKLOG_XLSX = _REPO_ROOT / "backlog.xlsx"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help=f"Workbook path (default: {DEFAULT_BACKLOG_XLSX.name} at repo root)",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    path = DEFAULT_BACKLOG_XLSX if args.path is None else args.path
