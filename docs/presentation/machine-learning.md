...
- **Shutter waste penalty** (production + nb08 default **off**, Exp 9 closeout): when `enable_shutter_waste_penalty`, accepted shutter with applied capture credit below `shutter_waste_reward_epsilon` → `shutter_waste_penalty = −k_shutter_waste` (`REWARD_SHUTTER_WASTE_PENALTY = 5.0`, tunable starting guess). Exp 7 fork ran with waste **on**; Exp 9 shows waste **off** + budget-exhausted **on** is better for SAC vector sparse.
