...
| D-015 | 2026-06-29 | deferred | Modular encoder — deferred high interest | **Revisit A0 vs A1 compress when `learning_mode` is achieved** — highly interesting if reward/credit/algorithm are fixed first | v1 not_supported in non-learnable slice; structured obs + lower A1 eval shutter rate; cannot test encoder benefit without learning | [Exp 6](../experiments/pipeline/0-initialized/06-modular-encoder-r2.md); Exp 2 Phase 4 §4.2 |
...
| D-021 | 2026-06-30 | deferred | Exp 6 encoder r2 | **Defer** modular encoder A0 vs A1 retest; archive charter | Low priority vs Exp 7; Exp 5 ruled out width; Exp 2 v1 not_supported | [0-06-modular-encoder-r2.md](../experiments/pipeline/99-archive/0-06-modular-encoder-r2.md) |
| D-022 | 2026-06-30 | accepted | MPO "KL explosion" = symptom; charter Exp 8 | KL/η blow-up is a **symptom** of an unconstrained M-step, **not** the cause; charter **Exp 8** (`ml_mpo_decoupled_dual`) to test the decoupled-KL MPO dual (E-step Q-dual η + separate α_μ/α_Σ trust region) as the MPO learning lever | KL ≈1e5 at first train ep while η≈4 (η lags); η frozen=1.0 still explodes KL + diverges critic; single `log_eta` conflates E-step temp with M-step trust region; width (D-018) & dense (D-007) already ruled out; SAC learns same sparse reward (D-017) | [mpo-learning-collapse-investigation.md](mpo-learning-collapse-investigation.md); [08-mpo-decoupled-dual-fix.md](../experiments/pipeline/0-initialized/08-mpo-decoupled-dual-fix.md); traces in `runs/*compare_mpo*`, `*mpo_model_size_mpo_s*`, `*h1b_mpo_stable_eta*` |
