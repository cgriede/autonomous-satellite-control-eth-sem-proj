...
\begin{itemize}
  ...
  \item \textbf{Exp~6:} Structured target-array encoding (A1) improves returns under the longer SAC sparse protocol (eval $+80.9$).
    Supported for the A1 vs.\ A0 encoder gap, not a baseline win: both arms remain below the matched warmup baseline and still spam the shutter.
  \item \textbf{Exp~7:} Budget-exhausted shutter penalty on vector-mode SAC yields the first budget-aware, selective positive eval return ($+10.6$) and cuts train shutter commands from $\sim$7000 (Exp~4) to 478.
