...
Exps~1--5 showed that neither threshold tuning, encoder architecture, algorithm choice, nor network
width were sufficient on their own.
The break came in Exp~3 (SAC learns under sparse reward) and was exploited in Exps~7 and~9, where
budget-exhausted penalty and waste-penalty removal each unlocked further learning.
Reward engineering, specifically what information is available at the action clock,
was the dominant design lever.

\paragraph{2.\ Action-space design before reward fine-tuning.}
The comparison between torque and vector pointing modes (Exps~4, 8, 11) shows that delegating
rate control to the onboard pointing controller is structurally important.
Vector mode removes safe-mode lockouts in the experiment runs (Section~\ref{sec:results-safemode}),
resolving a credit-assignment problem that no reward penalty could fix cleanly (Exp~10).
...
MPO's KL blow-up (Exps~3, 5) blocked all progress on the MPO branch until the root cause
(an unconstrained M-step multiplier) was identified and corrected (Exp~8).
The fix unlocked learning in vector mode (Exp~11) without any other design change.
Implementation correctness can masquerade as algorithm failure across multiple experiments.
That lesson applies before comparing reward or action-space variants.
...
Eval return on seed~7 exceeded the baseline anchor for the best SAC and MPO configurations, but the evaluation protocol is too narrow to claim robust mission superiority; the durable output of the semester is the mapped design space and the list of rejected shortcuts.
