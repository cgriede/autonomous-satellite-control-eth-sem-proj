...
\missingfigure[figwidth=\linewidth]{%
  \textbf{Fig.~0: Baseline overflight, three phases.}
  Overhead 2D view of one deterministic baseline episode.
  Phase~1 \textbf{Approach}: satellite enters the target corridor LOS window.
  Phase~2 \textbf{Point}: OBC PD loop slews toward target bearing; off-nadir angle grows.
  Phase~3 \textbf{Capture}: boresight error drops below threshold (Table~\ref{tab:baseline-config}), shutter fires.
  Repeat for each scheduled target regardless of cloud cover.
}
