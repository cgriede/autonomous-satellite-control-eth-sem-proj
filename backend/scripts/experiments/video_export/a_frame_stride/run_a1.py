"""Hypothesis A — frame_stride=2 vs shared baseline."""
...
PREVIEW_PATH = BRANCH_ROOT / "results" / "a1_preview.mp4"
ANALYSIS_PATH = EXPERIMENT_ROOT / "results" / "frame_stride_analysis.md"
...
def main() -> None:
    ...
    write_hypothesis_result(
        RESULT_PATH,
        experiment_id="a1_frame_stride",
        hypothesis_id="frame_stride",
        phase="A1",
        frozen_input={
            "description": f"Export frame_stride={FRAME_STRIDE} on high_cloud notebook fixture",
        ...
        treatment={
            "label": f"frame_stride_{FRAME_STRIDE}",
        ...
        instrumentation={"patch": "save_one_pass_video_30x", "frame_stride": FRAME_STRIDE},
        debug_examples={"baseline_video": baseline.get("video"), "treatment_video": treatment["video"]},
        files_changed=["a_frame_stride/render_export_fork.py"],
        verdict=verdict,
        closeout=closeout,
    )
    write_analysis_card(ANALYSIS_PATH, hypothesis_id="frame_stride", json_path=RESULT_PATH)
