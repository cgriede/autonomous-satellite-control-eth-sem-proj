"""Run SAC arms with flat (A0) vs compressed (A1) encoder."""
...
_overnight_spec = importlib.util.spec_from_file_location(
    "ml_overnight_runner_common_encoder",
...
EXPERIMENT_LOG = RESULTS_DIR / "modular_encoder.log"
...
def _make_sac_env(setup: tw.TrainingWorkflowSetup) -> Any:
...
def _attach_sac_agent(
    ...
    if spec.encoder_kind == "flat":
        ...
        return _replace_agent(setup, _sac_mod.SACAgent(env, config=setup.mpo_config))
...
def run_sac_arm(spec: ArmSpec, *, show_progress: bool = False, trim_artifacts: bool = False) -> dict[str, Any]:
    ...
    require_dt_profile(DT_15, context=f"ml_modular_encoder:{spec.arm_id}")
    ...
    cfg = frozen_training_config(
        run_id=f"ml_encoder_{spec.arm_id}_{stamp}",
    ...
    setup = _attach_sac_agent(setup, spec)
