"""Experiment-only reward patch: sparse + force torque effort in vector mode (Exp 12)."""
...
def activate_reward_fork(mode: RewardForkMode = "sparse") -> None:
    ...
    if mode != "sparse":
        raise ValueError(f"Exp 12 supports sparse only; got {mode!r}")
    if not _PATCHED:
...
def _patched_compute_reward(signals: Any, cfg: Any) -> tuple[float, dict[str, float]]:
    ...
    cfg = replace(cfg, enable_torque_effort=True)
