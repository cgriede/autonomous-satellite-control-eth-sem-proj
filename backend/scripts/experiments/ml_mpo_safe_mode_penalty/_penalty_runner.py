"""Exp 10: MPO fixed decoupled-KL dual, sparse reward + safe-mode penalty, torque @ dt 1.5s."""
...
from _penalty_frozen import penalty_config  # noqa: E402
from _profile_baseline import baseline_comparison, profile, shared_mpo_overrides  # noqa: E402
from _reward_fork import activate_reward_fork, safe_mode_penalty_enabled  # noqa: E402
...
from _profile_baseline import frozen_training_config  # noqa: E402
...
def _load_overnight_runner() -> Any:
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "ml_overnight_runner_common_mpo_safe_mode_penalty",
...
EXPERIMENT_LOG = RESULTS_DIR / "mpo_safe_mode_penalty.log"
ARM_ID = "safe_mode_penalty_on"
HYPOTHESIS_ID = "mpo_safe_mode_penalty"
EXPERIMENT_ID = "ml_mpo_safe_mode_penalty"
...
def _sync_run_config_mpo(setup: tw.TrainingWorkflowSetup) -> None:
    ...
    snapshot["experiment"] = {
        ...
        "reward_mode": "sparse",
        "enable_safe_mode_penalty": bool(p_cfg["enable_safe_mode_penalty"]),
        "k_safe_mode_penalty": float(p_cfg["k_safe_mode_penalty"]),
...
def _verify_run_config_dual(run_dir: Path, expected: dict[str, Any]) -> dict[str, Any]:
    ...
    if not exp.get("enable_safe_mode_penalty"):
        raise RuntimeError(f"enable_safe_mode_penalty not set in {config_path}")
    return mpo
...
def run_penalty_arm(
    *,
    ...
    set_warmup_fingerprint_extra(
        ...
        reward_mode="sparse",
        mpo_penalty_arm=ARM_ID,
        attitude_request_mode="torque",
        enable_safe_mode_penalty=p_cfg["enable_safe_mode_penalty"],
        k_safe_mode_penalty=p_cfg["k_safe_mode_penalty"],
        **mpo_overrides,
    )
    activate_warmup_fingerprint_patch()
    ...
    cfg = frozen_training_config(
        run_id=f"ml_mpo_safe_mode_penalty_{run_suffix}_{stamp}",
    ...
    if smoke:
        ...
        warmup = setup.runner.run_serial(
            ...
            feature_config=setup.feature_config,
            observation_layout=setup.observation_layout,
            np_rng=np.random.default_rng(derive_seed(7, "mpo_safe_mode_penalty_smoke", 0)),
        )
        ...
        return {
            ...
            "enable_safe_mode_penalty": safe_mode_penalty_enabled(),
            "k_safe_mode_penalty": p_cfg["k_safe_mode_penalty"],
        }
