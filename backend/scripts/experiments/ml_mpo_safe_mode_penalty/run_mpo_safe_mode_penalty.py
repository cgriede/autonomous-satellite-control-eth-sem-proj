"""Exp 10: MPO sparse torque + safe-mode reward penalty @ dt 1.5s."""
...
from _penalty_runner import (  # noqa: E402
    EXPERIMENT_ID,
    RESULTS_DIR,
    append_log,
    run_penalty_arm,
...
SUMMARY_JSON = RESULTS_DIR / "mpo_safe_mode_penalty.json"
...
def run_smoke(*, allow_cpu: bool = False) -> dict:
    ...
    payload = run_penalty_arm(smoke=True)
    write_hypothesis_result(
        SMOKE_JSON,
        phase="smoke",
    ...
    append_log(
        ...
        f"penalty_on={payload['enable_safe_mode_penalty']}"
    )
    return payload
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ml_mpo_safe_mode_penalty — Exp 10 MPO sparse torque + safe-mode penalty"
    )
    ...
    if args.smoke:
        ...
        print(
            f"  safe_mode_penalty={payload['enable_safe_mode_penalty']} "
            f"k={payload['k_safe_mode_penalty']}"
        )
        return
    ...
    append_log(f"RUN start train_episodes={train_episodes} baseline={profile().get('ref_run_id')}")

    try:
        kpis = run_penalty_arm(
            show_progress=args.show_progress,
            trim_artifacts=args.trim_artifacts,
            train_episodes=train_episodes,
        )
    except Exception as exc:
        err_path = RESULTS_DIR / "safe_mode_penalty_on_error.json"
        write_hypothesis_result(
            err_path,
            experiment_id=EXPERIMENT_ID,
    ...
    summary = {
        ...
        "enable_safe_mode_penalty": kpis.get("enable_safe_mode_penalty"),
        "k_safe_mode_penalty": kpis.get("k_safe_mode_penalty"),
        "arm": kpis,
    }
