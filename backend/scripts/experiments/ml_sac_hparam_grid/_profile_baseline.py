...
def size_preset(size: str) -> dict[str, Any]:
    presets = _PROFILE.get("size_presets") or {}
    if size not in presets:
        raise KeyError(f"Unknown size {size!r}; choose from {list(presets)}")
    return dict(presets[size])
...
__all__ = ["frozen_training_config", "profile", "shared_mpo_overrides", "size_preset"]
