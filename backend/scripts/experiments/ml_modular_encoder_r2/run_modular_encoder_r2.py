"""Exp 6: SAC — flat encoder (A0) vs vector-compress encoder (A1), r2 protocol."""
...
SUMMARY_JSON = RESULTS_DIR / "modular_encoder_r2_summary.json"
ANALYSIS_MD = EXPERIMENT_ROOT / "modular_encoder_r2_analysis.md"
...
def run_smoke(*, allow_cpu: bool = False, vector_embed_dim: int = 8) -> dict:
    ...
    require_dt_profile(DT_15, context=f"{EXPERIMENT_ID}:smoke")
    ...
    cfg = replace(
        frozen_training_config(
            run_id=f"ml_encoder_r2_smoke_{stamp}",
            train_episodes=0,
    ...
    setup = replace(
        setup,
        agent=SACModularAgent(
            env,
            config=setup.mpo_config,
    ...
    warmup = setup.runner.run_serial(
        ...
        feature_config=setup.feature_config,
        observation_layout=setup.observation_layout,
        np_rng=np.random.default_rng(derive_seed(EXPERIMENT_SEED, "encoder_r2_smoke", 0)),
    )
    ...
    append_log(f"SMOKE OK warmup_return={payload['warmup_return']:.1f} reward_mode={reward_mode}")
    return payload


def main() -> None:
    parser = argparse.ArgumentParser(description="ml_modular_encoder_r2 experiment (Exp 6)")
    ...
    parser.add_argument(
        ...
        help=f"Train episodes (profile default {default_train_episodes()})",
    )
    ...
    summary = {
        ...
        "profile_reward_mode": profile_reward_mode(),
    ...
    write_analysis_card(
        ANALYSIS_MD,
        hypothesis_id="modular_encoder_r2_sac",
        json_path=SUMMARY_JSON,
        sections={
            "Hypothesis": "Vector compressors on bearing/mask (A1) beat flat concat (A0) under r2 learnable protocol.",
