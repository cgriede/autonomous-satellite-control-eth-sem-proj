"""Duck-type workflow + MPO knobs from profile.json (Exp 6 r2 placeholder protocol)."""
...
_spec = importlib.util.spec_from_file_location(
    "ml_algo_overnight_frozen_baseline_encoder_r2",
...
WARMUP_EPISODES = _parent_mod.WARMUP_EPISODES
EVAL_EPISODES = _parent_mod.EVAL_EPISODES
BASELINE_WARMUP_RETURN_MEAN = _parent_mod.BASELINE_WARMUP_RETURN_MEAN
...
def profile_reward_mode() -> RewardMode:
    mode = str(_PROFILE.get("reward_mode", "sparse")).strip().lower()
    if mode not in ("sparse", "dense"):
        raise ValueError(f"Invalid profile reward_mode {mode!r}; choose sparse or dense")
...
def default_train_episodes() -> int:
    return int((_PROFILE.get("workflow") or {}).get("train_episodes", 50))
