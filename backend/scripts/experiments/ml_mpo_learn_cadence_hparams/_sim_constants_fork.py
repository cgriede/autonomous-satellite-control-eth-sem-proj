"""dt profiles for Exp 13 cadence arms (re-exports overnight fork + ap=3s profile)."""
...
_spec = importlib.util.spec_from_file_location(
    "ml_overnight_sim_constants_exp13",
...
DtProfile = _overnight_sim.DtProfile
DT_CANDIDATES = _overnight_sim.DT_CANDIDATES
DT_15 = next(p for p in DT_CANDIDATES if p.label == "dt_1.5s")
DT_15_AP2 = DtProfile(1.5, 3.0, 3.0, "dt_1.5s_ap2")
FIXED_COMPARE_PROFILE = _overnight_sim.FIXED_COMPARE_DT
...
def dt_profile_for_arm(*, sim_dt_s: float, controller_interval_s: float, label: str) -> DtProfile:
    return DtProfile(
        float(sim_dt_s),
        float(controller_interval_s),
        float(controller_interval_s),
        label,
    )


def write_dt_profile(
    *,
    train_episodes: int,
    sim_dt_s: float = 1.5,
    controller_interval_s: float = 1.5,
    label: str = "dt_1.5s",
) -> dict[str, Any]:
    payload = {
        "aborted": False,
        ...
        "source": "ml_mpo_learn_cadence_hparams",
    }
