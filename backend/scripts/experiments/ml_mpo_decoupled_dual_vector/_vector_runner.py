"""Exp 11: production MPOAgent with fixed decoupled-KL dual, sparse reward, vector mode."""
...
def _load_overnight_runner() -> Any:
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "ml_overnight_runner_common_mpo_dual_vector",
...
EXPERIMENT_LOG = RESULTS_DIR / "mpo_vector.log"
ARM_ID = "vector_sparse"
HYPOTHESIS_ID = "mpo_dual_vector_sparse"
...
def _sync_run_config_mpo(setup: tw.TrainingWorkflowSetup) -> None:
    ...
    snapshot["experiment"] = {
        ...
        "reward_mode": "sparse",
        "torque_comparator": profile().get("ref_run_id"),
    }
...
def _verify_run_config_dual(run_dir: Path, expected: dict[str, Any]) -> dict[str, Any]:
    ...
    if exp.get("experiment_id") != "ml_mpo_decoupled_dual_vector":
...
def run_vector_arm(
    *,
    ...
    if not smoke:
        acquire_experiment_run_lock(script="vector_sparse")
    append_log(f"START arm={ARM_ID} smoke={smoke}")

    dt_profile = apply_experiment_dt_profile()
    apply_dt_profile(DT_15)
    require_dt_profile(DT_15, context=f"ml_mpo_decoupled_dual_vector:{ARM_ID}")
    ...
    if smoke:
        ...
        warmup = setup.runner.run_serial(
            ...
            feature_config=setup.feature_config,
            observation_layout=setup.observation_layout,
            np_rng=np.random.default_rng(derive_seed(7, "mpo_dual_vector_smoke", 0)),
        )
