"""Exp 8: MPO fixed decoupled-KL dual, sparse reward, torque mode @ dt 1.5s."""
...
from _torque_runner import (  # noqa: E402
    RESULTS_DIR,
    append_log,
    run_torque_arm,
...
SUMMARY_JSON = RESULTS_DIR / "mpo_torque.json"
...
def run_smoke(*, allow_cpu: bool = False) -> dict:
    ...
    payload = run_torque_arm(smoke=True)
    write_hypothesis_result(
        SMOKE_JSON,
        phase="smoke",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ml_mpo_decoupled_dual_torque — Exp 8 MPO sparse torque"
    )
    ...
    try:
        ...
        err_path = RESULTS_DIR / "torque_sparse_error.json"
        write_hypothesis_result(
            err_path,
            experiment_id="ml_mpo_decoupled_dual_torque",
